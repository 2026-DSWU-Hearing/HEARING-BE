...
DeviceConnectionManager: 하드웨어 (device_id → 단일 연결)
"""
...
class DeviceConnectionManager:
    """하드웨어(ESP32) WS 연결 매니저. 기기당 연결 하나 — 재접속하면 기존 연결을 교체한다."""
    ...
    def disconnect(self, device_id: int, ws: WebSocket) -> bool:
    ...
    async def close_device(self, device_id: int) -> bool:
        """서버측 강제 종료 (PATCH is_connected=false). 살아있는 연결이 있었으면 True."""
