...
- `mac`: 기기 실제 MAC. 등록된 Device(mac_address)를 찾아 소유 유저에 바인딩.
  대소문자·공백 무관(등록·접속 양쪽 모두 정규화). 이미 등록된 MAC 재등록은 `409`.
...
- `PATCH /devices/{id}` 로 `is_connected=false` 수신 시 서버가 이 WS 를 닫음 (FE '연결 해제')
